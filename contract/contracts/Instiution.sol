...
contract Institution {
  ...
  event newStudentRegistration(address indexed _studentAddress, string _name, Student indexed _studentProfile);
  ...
  modifier registrationCompliance(address _studentAddress) {
    require(!isRegistered[_studentAddress], "Student has already registered");
    _;
  }
  ...
  function studentRegistration(
    ...
    uint _age
  ) external payable registrationCompliance(msg.sender) registrationFeeCompliance {
    uint id = counter++;
    ...
    Student newStudentProfile = new Student(newStudent.Address, newStudent.name, newStudent.age, newStudent.department);
    newStudentProfile.faculty;
    StudentProfile[msg.sender] = newStudentProfile;

    emit newStudentRegistration(newStudent.Address, newStudent.name, newStudentProfile);
  }
